...
const DataCharts: React.FC<DataChartsProps> = ({ stats, flaggedInvoices }) => {
  ...
  const amountProcessed =
    parseFloat(String(stats.amountProcessed).replace(/,/g, "")) || 0;
  const amountAtRisk =
    parseFloat(String(stats.amountAtRisk).replace(/,/g, "")) || 0;
  const amountSaved =
    parseFloat(String(stats.amountSaved).replace(/,/g, "")) || 0;
  ...
  const computeRecoveryStats = () => {
    ...
    for (let i = 0; i < types.length; i++)
      rawCounts.push({ total: 0, detected: 0, totalAmt: 0, detectedAmt: 0 });

    const pickAmount = (r: any) => {
      for (const c of amountCandidates) {
        if (c in r && r[c] != null && String(r[c]).trim() !== "")
          return parseFloat(String(r[c]).replace(/,/g, "")) || 0;
        ...
        for (const k of Object.keys(r)) {
          if (
            ...
            return parseFloat(String(r[k]).replace(/,/g, "")) || 0;
        }
      }
      return 0;
    };
    ...
    const getAny = (r: any, cands: string[]) => {
      for (const c of cands) {
        ...
        for (const k of Object.keys(r))
          if (k.toLowerCase() === c.toLowerCase() && r[k] != null) return r[k];
      }
      return undefined;
    };
            ...
            y: {
              ...
              ticks: {
                callback: function (val: any) {
                  return formatAmountCompact(Number(val));
                },
              },
            },
          },
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              label: (c: any) => {
                const v = c.raw || 0;
                if (v === 0) return `${c.label}: N/A`;
                return `${c.label}: ₹${formatAmountCompact(v)}`;
              },
            },
          },
        },
  ...
  useEffect(() => {
    if (chartInstances.current["hist"])
      chartInstances.current["hist"].destroy();
    const riskScores = (flaggedInvoices || [])
      .map((inv: any) => parseFloat(inv.risk_score || inv.risk || 0))
    ...
    const histCanvas = document.getElementById(
      histCanvasId,
  ...
  useEffect(() => {
    if (chartInstances.current["detection"])
      chartInstances.current["detection"].destroy();
    ...
    const ctx =
      detectionChartRef.current?.getContext &&
      detectionChartRef.current.getContext("2d");
    if (!ctx) return;
          ...
          tooltip: {
            callbacks: {
              label: (ctx: any) => {
                ...
                const rc =
                  recovery.rawCounts && recovery.rawCounts[idx]
                    ? recovery.rawCounts[idx]
                    : null;
    ...
    <section className="charts-grid">
      <div className="chart-card" style={{ minHeight: 260 }}>
        ...
        <div
          ...
          style={{ padding: "1rem", height: "220px" }}
        >
          ...
          >
            ...
            >
              ...
              <strong style={{ fontSize: "1.2rem", color: "#fff" }}>
                ₹{formatAmountCompact(amountProcessed)}
              </strong>
            </div>
          </div>
        </div>
            ...
            return (
              <>
                {types.map((t: any, i: number) => {
                  const rc =
                    rawCounts && rawCounts[i] ? rawCounts[i] : { detected: 0 };
                  return (
                    <div
                      key={t.key}
                      style={{
                  ...
                  <div style={{ fontSize: "0.85rem" }}>
                    ...
                    {Math.max(
                      ...
                      ((stats && stats.totalInvoices) || 0) -
                        (computeRecoveryStats().rawCounts || []).reduce(
                          (s: number, r: any) => s + (r.detected || 0),
                          0,
                        ),
                    )}
                  </div>
