...
const DashboardStats: React.FC<DashboardStatsProps> = ({ stats }) => {
  const amountAtRiskNum =
    typeof stats.amountAtRisk === "string"
      ? parseFloat(stats.amountAtRisk.replace(/,/g, ""))
      : stats.amountAtRisk;
  const amountSavedNum =
    typeof stats.amountSaved === "string"
      ? parseFloat(stats.amountSaved.replace(/,/g, ""))
      : stats.amountSaved;

  const recoveryRate =
    amountAtRiskNum > 0
      ? ((amountSavedNum / amountAtRiskNum) * 100).toFixed(1)
      : 100;

  const total = parseInt(stats.totalInvoices.toString()) || 0;
  const hold = parseInt(stats.autoHold?.toString() || "0");
  const review = parseInt(
    stats.manualReview?.toString() || Math.floor(total * 0.1).toString(),
  );
    ...
    <>
      <div className="kpi-row">
        ...
        <div className="kpi red">
          ...
          <div className="kpi-val sm">
            ₹{formatCurrency(stats.amountAtRisk)}
          </div>
      ...
      <div className="dec-row">
        ...
        <div className="dec-card amber">
          ...
          <div className="dec-bar">
            <div
              className="dec-fill"
              style={{ width: `${total ? (review / total) * 100 : 0}%` }}
            ></div>
          </div>
        </div>
        <div className="dec-card red">
          ...
          <div className="dec-bar">
            <div
              className="dec-fill"
              style={{ width: `${total ? (hold / total) * 100 : 0}%` }}
            ></div>
          </div>
        </div>
      </div>
