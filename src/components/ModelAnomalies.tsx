...
const KNOWN_LABELS: { key: string; aliases: string[] }[] = [
  ...
  {
    key: "Rate Mismatch",
    aliases: ["rate mismatch", "rate_mismatch", "rate-mismatch"],
  },
  { key: "Ghost", aliases: ["ghost", "ghost invoice"] },
  {
    key: "3-Way Match Fail",
    aliases: [
      "3-way",
      "3 way",
      "3-way match",
...
const findTypeKey = (inv: any) => {
  ...
  for (const k of keys)
    if (/type|reason|issue|category|anomaly/i.test(k)) return k;
  return null;
};
  ...
  return (
    <div style={{ marginBottom: "1.5rem" }}>
      ...
      >
        ...
        {["Duplicate", "Rate Mismatch", "Ghost", "3-Way Match Fail"].map(
          (k) => (
            <div
              key={k}
              style={{
